...
void CMOEAD::evol_population()
{
	...
	//t_ini=clock();
//printf("Tamaño del vector order: %d \n",order.size());
    for(int sub=0; sub<order.size(); sub++)
	{
		...
		child.obj_eval();
		//t_fin=clock();
		//result=(double)(t_fin-t_ini)/CLOCKS_PER_SEC;
		//printf("Tiempo (milisegundos) de la función obj_eval: %.16g \n",result*1000.0);
...
//printf("prueba result: %.16g\n", result);
//contador++;
//printf("Tiempo (milisegundos) del bucle del método evol_population: %.16g, ejecución: %d \n",result*1000.0,contador);
}
...
//	max_gen = int(5.0*300000/pops);
	...
	//printf("Tiempo del bucle de la función logistic_score: %.16g\n",result);

...
double fitnessfunction(solution_t solucion, vector <double> &namda)
{
	...
	{
		double diff = fabs(solucion.score[n] - idealpoint[n]); //fabs calcula el valor absoluto de un num en coma flotante.
		double feval;
		if(namda[n]==0) 
			feval = 0.0001*diff;
		else
